...
export default function NavBar({ activePath, items = defaultNavItems }: NavBarProps) {
  ...
  return (
    <nav className="bg-ink-900/80 backdrop-blur-md sticky top-0 z-50 border-b border-border/50">
      <div className="max-w-6xl mx-auto px-6 py-2 flex items-center justify-between">
        <div className="flex items-center gap-10">
          <Link to="/" className="flex items-center py-3" onClick={handleLogoClick}>
            <Logo size="md" />
          </Link>
        ...
        >
          ...
          <span className="hidden sm:inline">Offrir un cafe</span>
        </Button>
      </div>
    </nav>
  );
